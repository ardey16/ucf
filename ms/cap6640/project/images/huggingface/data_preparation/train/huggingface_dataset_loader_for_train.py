...
tee = Tee("huggingface_dataset_loader_for_output.txt")
sys.stdout = tee
...
# Load the required dataset (JamieWithofs/Deepfake-and-real-images) from Hugging Face, for the train set
dataset = load_dataset("JamieWithofs/Deepfake-and-real-images", split="train", cache_dir=hf_home)
...
os.makedirs(f"{cap6640_project_dir}/Images/HuggingFace/JamieWithofs/Deepfake-and-real-images/train", exist_ok=True)

# Get the unique labals for each entry in the dataset
labels = set(entry["label"] for entry in dataset)
for label in labels:
    # Create label subdirectories
    os.makedirs(f"{cap6640_project_dir}/Images/HuggingFace/JamieWithofs/Deepfake-and-real-images/train/{label}", exist_ok=True)
...
for idx, entry in enumerate(dataset):
    ...
    with open(f"{cap6640_project_dir}/Images/HuggingFace/JamieWithofs/Deepfake-and-real-images/train/{label}/image_{idx}.jpg", "wb") as f:
        f.write(image_bytes)
