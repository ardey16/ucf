...
tee = Tee("huggingface_dataset_loader_for_test_output.txt")
sys.stdout = tee
...
# Load the required dataset (JamieWithofs/Deepfake-and-real-images) from Hugging Face, for the test set
dataset = load_dataset("JamieWithofs/Deepfake-and-real-images", split="test", cache_dir=hf_home)
...
for label in labels:
    ...
    os.makedirs(f"{cap6640_project_dir}/Images/HuggingFace/JamieWithofs/Deepfake-and-real-images/test/{label}", exist_ok=True)
...
for idx, entry in enumerate(dataset):
    ...
    with open(f"{cap6640_project_dir}/Images/HuggingFace/JamieWithofs/Deepfake-and-real-images/test/{label}/image_{idx}.jpg", "wb") as f:
        f.write(image_bytes)
