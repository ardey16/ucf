...
tee = Tee("openapi_deepfake_image_detector_for_test_10905_output.txt")
sys.stdout = tee
...
for filepath in file_paths:
    ...
    if filepath.is_file():
        ...
        if (second_last_part == "0") and (deepfake_detected in response.choices[0].message.content):
            num_true_positive += 1
        # False Positive - the image is not deepfake but is detected deepfake
        elif (second_last_part == "1") and (deepfake_detected in response.choices[0].message.content):
            num_false_poitive += 1
        # False Negative - the image is deepfake but is not detected deepfake
        elif (second_last_part == "0") and not (deepfake_detected in response.choices[0].message.content):
