...
tee = Tee("huggingface_deepfake_image_detector_2_for_test_1000_output.txt")
sys.stdout = tee
...
for filepath in file_paths:
    ...
    if filepath.is_file():
        ...
        image_as_base64_encoded_string = image_to_base64(
            image_path)  # Convert image to base64 encoded string
        ...
        # Send the image to HuggingFace Inference API for analysis
        response = client.chat.completions.create(
             ...
             messages=[
                  {"role": "system", "content": "You are an image forensic expert that can detect a deepfake image."},
                  {
                       ...
                       "content": [
                            {"type": "text",
                             "text": "Tell me if synthesis artifacts are in the image. You must return with 1) yes or no only; 2) if yes, explain in a single short sentence where the artifacts exist by answering in [region, artifacts] form - use printable characters only."},
                             {
                                  "type": "image_url",
    ...
    print(
        f"{current_time.strftime("%Y-%m-%d %H:%M:%S"):<25} {current_image_number:<10} / {total_image_count:<10} {last_part:<25} Label? {label:<20} Deepfake detected? {response.choices[0].message.content}")
...
print(
    f"Time taken to complete processing {current_image_number} images: {(end_time - start_time):.0f} seconds")
